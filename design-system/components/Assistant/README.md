...
Store the file in the design system's asset store and reference it from there, so every surface loads the same crop.

## Mira is labelled, always

**A photographic human face beside a message reads as a person.** Wherever the portrait appears, the name **Mira** and the role **MIDTRANS logistics assistant** appear with it — in the header, in the first message of a conversation, and in any notification she sends. This is not a disclaimer to be tucked away: it is what stops a customer believing an operator promised them something.
